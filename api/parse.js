...
2. restaurant_id : prends l'id exact de la liste. "amiens cv", "acv", "centre ville" = Amiens Centre-ville. "bsm" = Boulogne Saint-Martin. Si le sujet concerne l'ensemble du groupe ("tous les restaurants", "partout", "sur les 7 sites"), mets restaurant_id à 0. Si aucun restaurant n'est identifiable, mets null et ajoute "restaurant" dans incertain — ne choisis JAMAIS un restaurant au hasard ou par défaut : mieux vaut null que le mauvais site. Un sujet de siège, de direction ou de travail de fond ("terminer le fichier X", "préparer le budget") n'a normalement pas de restaurant : null.
...
4. titre : une phrase courte et actionnable, 80 caractères max, qui commence par un verbe quand c'est une action ("Demander l'accès aux caméras") ou par le constat quand c'est un incident ("Friteuse 2 en panne"). Garde ses mots à elle. Écris-le du point de vue de celui qui doit agir : retire la formule d'attribution et bascule les possessifs à la première personne — "Thomas doit revoir son planning" devient "Revoir mon planning". Le destinataire est porté par assigne_id, il n'a pas à figurer dans le titre.
...
10. actions : découpe en étapes dès qu'elle énumère plusieurs gestes pour un même sujet ("appeler l'assureur, envoyer le constat et relancer vendredi" = un sujet, trois étapes), ou quand elle décrit une démarche qui en comporte manifestement plusieurs et qu'elle les nomme. N'invente jamais d'étapes qu'elle n'a pas dites. Sinon tableau vide.
11. assigne_id : si elle parle d'elle-même comme devant agir ("je dois", "il faut que je", "je m'en occupe"), mets son propre id (voir "Qui te parle"). Sinon, dès qu'un utilisateur de l'app est cité comme celui qui doit agir, mets son id. Toutes les tournures comptent : "pour Thomas", "voir avec Agnès", "demander le planning à Pauline", "je délègue à X", "X doit relancer", "faire faire par X". La règle simple : si la phrase désigne quelqu'un d'autre que celui qui parle comme devant s'en occuper, c'est lui l'assigné. Ne confonds pas un utilisateur de l'app avec un membre d'équipe en restaurant : un membre d'équipe va dans membre_id, jamais dans assigne_id.
12. prive : true par défaut dès qu'un assigne_id est renseigné — y compris quand elle se confie la tâche à elle-même ("je dois terminer ce dossier"), qui ne regarde alors qu'elle — confier un sujet à quelqu'un est une conversation à deux. Mets false si la phrase dit que ça concerne tout le monde ("pour tous", "toute l'équipe", "à voir en réunion"). true aussi si elle dit que c'est personnel ("perso", "pour moi") — le sujet n'est alors vu que par elle — ou si elle restreint le sujet à la personne assignée ("juste entre Thomas et moi", "ne pas diffuser", "confidentiel"), auquel cas renseigne aussi assigne_id : seuls elle et cette personne verront le sujet.
